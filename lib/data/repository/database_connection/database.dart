...
import 'package:manektech_task/data/modal/cart_modal.dart';
...
class DatabaseConnection {
  ...
  openDatabaseConnection() async {
    _db = await openDatabase('demo.db', version: 1,
        onCreate: (Database db, int version) async {
      // When creating the db, create the table
      await db.execute(
          'CREATE TABLE cart (id INTEGER PRIMARY KEY, product TEXT, qty INTEGER)');
    });
  }
  ...
  Future<int?> addItemToCart(CartModal cartItem) async {
    ...
    try {
      return await _db?.rawInsert('INSERT INTO cart(product,qty) VALUES(?,?)',
          [json.encode(cartItem.product.toJson()), cartItem.qty]);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<int?> updateItemInCart(CartModal cartItem) async {
    ...
    try {
      return await _db?.rawInsert(
          'UPDATE cart SET qty=? WHERE id=?', [cartItem.qty, cartItem.cartId]);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<int?> deleteItemFromCart(CartModal cartItem) async {
    ...
    try {
      return await _db
          ?.rawInsert('DELETE FROM cart WHERE id=?', [cartItem.cartId]);
    } catch (e) {
      rethrow;
    }
  }
